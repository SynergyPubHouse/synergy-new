...
const JICSNavigation = () => {
		...
		},
		{
			...
			dropdown: [
				...
				},
				{
					...
					// Change path to the file in public folder
					path: "/sample.docx",
					icon: (
						<svg
					...
					download: true, // Custom flag to indicate download
				},
			],
		},
		{
	...
	return (
		<nav className="bg-white shadow-lg rounded-lg border-b border-gray-200">
						...
						{navigationItems.map((item) => (
							...
							>
								<Link
									...
									className={`inline-flex items-center px-6 py-2 text-base font-semibold transition-colors duration-200 ${
										...
										(item.dropdown &&
											...
											: "text-gray-700 hover:text-indigo-700 hover:border-b-2 hover:border-indigo-700"
									}`}
								>
									{item.label}
									{item.dropdown && (
								...
								{item.dropdown && activeDropdown === item.label && (
									<div className="absolute left-0 mt-1 w-72 rounded-lg shadow-xl bg-white ring-1 ring-black ring-opacity-5 z-50 transform transition-all duration-200 ease-out">
										<div className="py-2">
												...
												if (dropdownItem.external) {
													return (
														<a
															...
															className="flex items-center px-6 py-3 text-base text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors duration-200"
														>
															{dropdownItem.label}
															{dropdownItem.icon}
														</a>

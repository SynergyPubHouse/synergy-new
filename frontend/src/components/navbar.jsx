import { useState, useEffect, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { FaUserCircle, FaBars, FaTimes, FaSearch } from 'react-icons/fa';
import { useAuth } from '../App';

const BASE = '/journal/Journal-of-Intelligent-Computing-Systems';

const Navbar = () => {
  const { user, logout } = useAuth();
  const [menuOpen, setMenuOpen] = useState(false);
  const [searchOpen, setSearchOpen] = useState(false);
  const [userDropdownOpen, setUserDropdownOpen] = useState(false);
  const location = useLocation();
  const navigate = useNavigate();
  const dropdownRef = useRef(null);

  const handleLogout = () => {
    if (window.confirm("Are you sure you want to log out?")) {
      logout();
      navigate("/");
    }
  };

  const navItems = [
    { name: 'Publish With Us', path: `${BASE}/publish` },
    { name: 'For Reviewers', path: `${BASE}/reviewers` },
    { name: 'Track Your Research', path: `${BASE}/track` }
  ];

  useEffect(() => {
    const handleClickOutside = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        setUserDropdownOpen(false);
      }
    };

    document.addEventListener('mousedown', handleClickOutside);
    return () => {
      document.removeEventListener('mousedown', handleClickOutside);
    };
  }, []);

  return (
    <motion.nav 
      className="p-4 shadow-lg fixed w-full z-50 transition-all bg-opacity-90 backdrop-blur-md bg-[#1f3247] text-white"
      initial={{ y: -50, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      transition={{ duration: 0.5 }}
    >
      <div className="container mx-auto flex justify-between items-center">
        <Link to={BASE} className="text-2xl font-bold">PaperSphere</Link>
        
        <div className="hidden md:flex gap-6">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`relative font-medium transition-all hover:text-[#BAFFF5] ${location.pathname === item.path ? 'text-[#BAFFF5] font-bold' : ''}`}
            >
              {item.name}
              {location.pathname === item.path && (
                <motion.div
                  layoutId="underline"
                  className="absolute left-0 -bottom-1 w-full h-1 bg-[#BAFFF5] rounded"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ duration: 0.3 }}
                />
              )}
            </Link>
          ))}
        </div>
        
        <div className="flex items-center gap-4">
          {user ? (
            <div className="relative" ref={dropdownRef}>
              <button 
                onClick={() => setUserDropdownOpen(!userDropdownOpen)} 
                className="flex items-center gap-2 focus:outline-none"
              >
                {user?.photoURL ? (
                  <img 
                    src={user.photoURL} 
                    alt="User" 
                    className="w-10 h-10 rounded-full border-2 border-[#BAFFF5]" 
                  />
                ) : (
                  <FaUserCircle className="w-10 h-10 text-[#BAFFF5]" />
                )}
                <span className="text-sm">{user?.name}</span>
              </button>

              <AnimatePresence>
                {userDropdownOpen && (
                  <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }}
                    transition={{ duration: 0.2 }}
                    className="absolute right-0 mt-2 w-48 bg-[#496580] rounded-lg shadow-lg py-2 text-white"
                  >
                    <div className="px-4 py-2 text-sm text-white">
                      <p className="font-semibold">{user.name}</p>
                      <p className="opacity-80">{user.email}</p>
                    </div>
                    <hr className="border-[#496580] border-opacity-20 my-2" />
                    <Link
                      to={`${BASE}/account`}
                      className="block px-4 py-2 text-sm hover:bg-[#BAFFF5] hover:text-[#496580] transition-colors"
                    >
                      My Account
                    </Link>
                    <Link
                      to={`${BASE}/subscriptions`}
                      className="block px-4 py-2 text-sm hover:bg-[#BAFFF5] hover:text-[#496580] transition-colors"
                    >
                      My Subscriptions
                    </Link>
                    <Link
                      to={`${BASE}/settings`}
                      className="block px-4 py-2 text-sm hover:bg-[#BAFFF5] hover:text-[#496580] transition-colors"
                    >
                      Settings
                    </Link>
                    <button 
                      onClick={handleLogout} 
                      className="w-full text-left px-4 py-2 text-sm hover:bg-[#BAFFF5] hover:text-[#496580] transition-colors"
                    >
                      Logout
                    </button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
          ) : (
            <div className="hidden md:flex gap-4">
              <Link to={`${BASE}/login`} className="text-white font-semibold px-4 py-2 rounded-md border border-[#BADDFF] border-opacity-30 transition-all hover:bg-white hover:text-[#496580] hover:border-[#BADDFF] hover:bg-opacity-10">
                Login
              </Link>
              <Link to={`${BASE}/register`} className="bg-[#BAFFF5] text-[#496580] font-semibold px-5 py-2 rounded-md shadow-md transition-all hover:bg-[#a8e6dc]">
                Register
              </Link>
            </div>
          )}
          
          <button className="md:hidden text-xl" onClick={() => setMenuOpen(!menuOpen)}>
            {menuOpen ? <FaTimes /> : <FaBars />}
          </button>
        </div>
      </div>
      
      {menuOpen && (
        <motion.div 
          initial={{ opacity: 0, y: -20 }} 
          animate={{ opacity: 1, y: 0 }} 
          transition={{ duration: 0.3 }} 
          className="md:hidden absolute top-16 left-0 w-full bg-[#496580] shadow-lg py-4 flex flex-col items-center gap-4 text-grey-200"
        >
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`text-lg font-medium transition-all hover:text-[#496580] ${location.pathname === item.path ? 'font-bold' : ''}`}
              onClick={() => setMenuOpen(false)}
            >
              {item.name}
            </Link>
          ))}
          {user ? (
            <>
              <Link
                to={`${BASE}/account`}
                className="text-lg font-medium transition-all hover:text-[#496580]"
                onClick={() => setMenuOpen(false)}
              >
                My Account
              </Link>
              <Link
                to={`${BASE}/subscriptions`}
                className="text-lg font-medium transition-all hover:text-[#496580]"
                onClick={() => setMenuOpen(false)}
              >
                My Subscriptions
              </Link>
              <Link
                to={`${BASE}/settings`}
                className="text-lg font-medium transition-all hover:text-[#496580]"
                onClick={() => setMenuOpen(false)}
              >
                Settings
              </Link>
              <button
                onClick={handleLogout}
                className="text-lg font-medium transition-all hover:text-[#496580]"
              >
                Logout
              </button>
            </>
          ) : (
            <div className="flex flex-col gap-4 w-full px-4">
              <Link 
                to={`${BASE}/login`} 
                className="text-center text-[#496580] hover:text-[#3a5269] font-medium py-2 transition-colors" 
                onClick={() => setMenuOpen(false)}
              >
                Login
              </Link>
              <Link 
                to={`${BASE}/register`} 
                className="bg-[#BAFFF5] text-[#496580] text-center px-4 py-2 rounded-lg hover:bg-[#a8e6dc] font-medium transition-colors" 
                onClick={() => setMenuOpen(false)}
              >
                Register
              </Link>
            </div>
          )}
        </motion.div>
      )}
    </motion.nav>
  );
};

export default Navbar;
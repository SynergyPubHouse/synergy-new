...
function EditorDashboard() {
  ...
  const [separateIssue, setSeparateIssue] = useState(false);
  ...
  const handleOpenPublishModal = (manuscript, isSeparateIssue = false) => {
    setUploadManuscript(manuscript);
    setSeparateIssue(isSeparateIssue);
  ...
  const handleDirectStatusUpdate = async (manuscriptId, newStatus) => {
    try {
      ...
      try {
        ...
        console.warn(
          "Failed to refresh manuscripts after direct status update:",
          e,
        );
      }
    } catch (error) {
      ...
      if (
        ...
        addToast(
          `Failed to update manuscript status to "${newStatus}"`,
          "error",
        );
      }
    }
  ...
  const handleBulkStatusUpdate = async (
    ...
    try {
      ...
      if (
        ...
        addToast(
          `Failed to bulk update manuscripts to "${newStatus}"`,
          "error",
        );
      }
    }
  ...
  const handleRevisionRequired = async (manuscriptId) => {
    try {
      ...
      addToast(
        response.data?.message || "Revision required note added successfully",
        "success",
      );
      ...
      try {
        ...
        console.warn(
          "Failed to refresh manuscripts after revision required:",
          e,
        );
      }
    } catch (error) {
      ...
      // Check for specific error about rejected manuscripts
      const errorMessage =
        error.response?.data?.message || "Failed to add revision required note";
  ...
  const handleAcceptManuscript = async () => {
    try {
      ...
      // Update local state
      const updatedManuscripts = manuscripts.map((m) =>
        m._id === selectedManuscript._id ? { ...m, status: "Accepted" } : m,
      );
      setManuscripts(updatedManuscripts);
  ...
  const calculateStatistics = useCallback(() => {
    ...
    if (statisticsDateRange.startDate && statisticsDateRange.endDate) {
      ...
      filteredManuscripts = allManuscripts.filter((manuscript) => {
        const manuscriptDate = new Date(
          manuscript.submissionDate || manuscript.createdAt,
        );
    ...
    // Revision statistics
    const revisionStats = filteredManuscripts.reduce(
      (acc, manuscript) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setManuscripts((prev) =>
        prev.filter((m) => m._id !== deleteManuscript._id),
      );
      ...
      setUsers((prevUsers) =>
        prevUsers.map((u) => ({
          ...u,
          manuscripts: (u.manuscripts || []).filter(
            (m) => m._id !== deleteManuscript._id,
          ),
        })),
      );

      addToast(
        `Manuscript "${deleteManuscript.title}" deleted successfully!`,
        "success",
      );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg p-6 shadow-md border border-[#e2e8f0] col-span-2">
                ...
                {filterType !== "all" && selectedUser && (
                  <div className="flex items-center space-x-2 mt-2">
                    ...
                    <span className="text-xs text-gray-500">
                      ({getFilteredManuscripts().length} manuscripts)
                    </span>
                  </div>
                )}
              </div>
            ...
            {/* Bulk Actions Panel */}
            {showBulkActions && getFilteredManuscripts().length > 0 && (
              <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  <button
                    onClick={() =>
                      setSelectedManuscriptIds(
                        getFilteredManuscripts().map((m) => m._id),
                      )
                    }
                ...
                {selectedManuscriptIds.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() =>
                        handleBulkStatusUpdate(selectedManuscriptIds, "Pending")
                      }
                      className="px-3 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600"
                    >
                    ...
                    >
                      📝 Revision Required ({selectedManuscriptIds.length})
                    </button>
                    <button
                      onClick={() =>
                        handleBulkStatusUpdate(
                ...
                >
                  <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-4 items-start">
                    <div className="flex items-start space-x-3 min-w-0">
                      ...
                      {showBulkActions && (
                        <input
                          ...
                          onChange={() =>
                            toggleManuscriptSelection(manuscript._id)
                          }
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xl font-semibold text-[#1a365d] mb-2 break-words">
                          {manuscript.title}
                        </h3>
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-3 mb-2">
                          ...
                          <p className="text-[#00796b] text-sm font-medium sm:text-right">
                            ID:{" "}
                            {manuscript.customId ||
                              manuscript._id.slice(-6).toUpperCase()}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {typeof manuscript.revisionAttempts === "number" && (
                            <span className="text-xs text-[#496580]">
                              Attempts: {manuscript.revisionAttempts}/
                              {manuscript.maxRevisionAttempts || 3}
                            </span>
                          )}
                        </div>
                        {manuscript.revisionLocked && (
                          <p className="text-xs text-red-600 font-semibold mb-2">
                            ⚠ All revision attempts exhausted. Manuscript
                    ...
                    <div className="flex flex-col space-y-2 w-full">
                      ...
                      <button
                        onClick={() => handleInviteReviewers(manuscript)}
                      ...
                      <div className="bg-gray-50 p-3 rounded border border-gray-200">
                        ...
                        <div className="grid grid-cols-1 gap-1">
                          ...
                          {/* Accept */}
                          <button
                            onClick={() => handleAcceptClick(manuscript)}
                            className={`px-3 py-1 text-sm rounded ${
                              manuscript.status === "Accepted" ||
                              manuscript.status === "Published"
                                ? "bg-gray-400 text-white cursor-not-allowed"
                          ...
                          <button
                            onClick={() =>
                              handleActionClick(manuscript, "reject")
                            }
                            className={`px-3 py-1 text-sm rounded ${
                              manuscript.status === "Rejected"
                                ? "bg-gray-400 text-white cursor-not-allowed"
                          ...
                          {manuscript.status === "Published" ? (
                            <button
                              className="px-3 py-1 text-sm rounded text-white bg-gray-500 cursor-not-allowed"
                              disabled
                            >
                              {manuscript.separateIssue === true
                                ? "✓ Published as Special Issue"
                                : "✓ Published"}
                            </button>
                          ) : (
                            <>
                              <button
                                onClick={() =>
                                  handleOpenPublishModal(manuscript, true)
                                }
                                className="px-3 py-1 text-sm rounded text-white bg-yellow-600 hover:bg-yellow-700"
                              >
                                ⭐ Publish as Special Issue
                              </button>

                              <button
                                onClick={() =>
                                  handleOpenPublishModal(manuscript, false)
                                }
                                className="px-3 py-1 text-sm rounded text-white bg-indigo-500 hover:bg-indigo-600"
                              >
                                Publish
                              </button>
                            </>
                  ...
                  {(manuscript.editorNotes?.length > 0 ||
                    ...
                    <div className="notes-section mt-4 border-t border-[#e2e8f0] pt-4">
                      ...
                      <div className="space-y-3 max-h-60 overflow-y-auto">
                        ...
                        {(() => {
                          const isExpanded = expandedNotes[manuscript._id];
                          const allNotes = [
                            ...(manuscript.editorNotes || [])
                              .filter((note) => {
                                ...
                                return (
                                  ...
                                  !text.includes("required: function") &&
                          ...
                          return displayNotes.map((item, index) => {
                            ...
                            } else if (item.type === "editorForAuthor") {
                              return (
                                <div
                            ...
                            <button
                              ...
                              onClick={() => {
                                setExpandedNotes((prev) => ({
                                  ...prev,
                                  [manuscript._id]: !prev[manuscript._id],
                                }));
                              }}
                            >
                              {isExpanded
                  ...
                  {/* Note Input Section */}
                  {showNoteInput &&
                    selectedManuscript?._id === manuscript._id && (
                        ...
                        {showNoteInput === "reject" && (
                          <>
                            ...
                            <div className="flex justify-end space-x-2">
                              ...
                              <button
                                onClick={() =>
                                  handleStatusUpdate(manuscript._id, "Rejected")
                                }
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-[#e2e8f0]">
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-600 mt-3">
                💡 <strong>Tip:</strong> Add multiple reviewer email addresses
                above. Each reviewer will receive an invitation email with a
                link to register/login and accept or reject the review
                invitation.
              </p>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-between items-center mt-2">
                <p className="text-sm text-gray-500">
                  💬 This note will be included in the invitation email and
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 text-center shadow-2xl">
            ...
            <p className="text-gray-600 mb-4">
              Please wait while we send invitations to the selected reviewers...
            </p>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-4 rounded-xl shadow-lg">
                      <div className="text-3xl font-bold">
                        {statisticsData.totalManuscripts}
                      </div>
                      <div className="text-blue-100">Total Manuscripts</div>
                      <div className="text-xs text-blue-200 mt-1">
                        {statisticsData.dateRange.start &&
                        statisticsData.dateRange.end
                    ...
                    <div className="bg-gradient-to-br from-green-500 to-green-600 text-white p-4 rounded-xl shadow-lg">
                      <div className="text-3xl font-bold">
                        {(statisticsData.statusCounts["Accepted"] || 0) +
                          (statisticsData.statusCounts["Published"] || 0)}
                      </div>
                    ...
                    <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-4 rounded-xl shadow-lg">
                      <div className="text-3xl font-bold">
                        {statisticsData.avgProcessingDays}
                      </div>
                    ...
                    <div className="bg-gradient-to-br from-purple-500 to-purple-600 text-white p-4 rounded-xl shadow-lg">
                      <div className="text-3xl font-bold">
                        {statisticsData.revisionStats.manuscriptsWithRevisions}
                      </div>
                      <div className="text-purple-100">With Revisions</div>
                      <div className="text-xs text-purple-200 mt-1">
                        {statisticsData.revisionStats.totalRevisions} total
                        revision attempts
                      </div>
                    </div>
                  </div>
                                ...
                                <td className="px-4 py-3 text-center">
                                  <span
                                    className={`px-2 py-1 rounded-full font-semibold ${
                                      stats.total > 0 &&
                                      stats.accepted / stats.total >= 0.5
                                        ? "bg-green-100 text-green-800"
                                        : "bg-gray-100 text-gray-800"
                                    }`}
                                  >
                          ...
                          {statisticsData.filteredManuscripts
                            ...
                            .map((manuscript, index) => (
                              ...
                              >
                                ...
                                <td
                                  className="px-3 py-2 text-gray-800 font-medium max-w-xs truncate"
                  ...
                  <div className="flex justify-end space-x-3">
                    <button
                      onClick={() => {
                        ...
                        const rows = statisticsData.filteredManuscripts.map(
                          (m) => [
            ...
            <div className="p-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start">
                      ...
                      <span className="text-gray-800 font-mono text-sm">
                        {deleteManuscript.customId ||
                          deleteManuscript._id.slice(-6).toUpperCase()}
                      </span>
                    </div>
                    <div className="flex items-start">
                    ...
                    <div className="flex items-start">
                      <span className="text-gray-500 w-20 text-sm">
                        Status:
                      </span>
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-semibold ${
                          deleteManuscript.status === "Pending"
                            ? "bg-blue-100 text-blue-800"
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                <p className="text-red-700 text-sm flex items-start space-x-2">
                  <span>⚠️</span>
                  <span>
                    <strong>Warning:</strong> All associated files, notes, and
                    review history will be permanently deleted.
                  </span>
                </p>
              </div>
            </div>

...
// Helper: Convert DOCX to PDF using LibreOffice only
async function convertDocxToPdf(docxPath) {
	...
	return new Promise((resolve, reject) => {
		...
		console.log(
			`[convertDocxToPdf] Converting ${docxPath} to ${outputPdf}`
		);
		...
		shell.on("message", (message) => {
			console.log(`[convertDocxToPdf] Output: ${message}`);
			output.push(message);
		});
		...
		shell.end(async (err, code, signal) => {
			if (err) {
				...
				if (errorOutput.length > 0) {
					console.error(
						"[convertDocxToPdf] PythonShell stderr collected:",
						errorOutput.join("\n")
					);
				}
				return reject(
			...
			while (tries < maxTries) {
				try {
					if (
						fsSync.existsSync(outputPdf) &&
						fsSync.statSync(outputPdf).size > 100
					) {
						console.log(
							`[convertDocxToPdf] PDF created successfully: ${outputPdf}`
						);
						return resolve(outputPdf);
					}
				} catch (e) {
					console.error(
						`[convertDocxToPdf] Error checking PDF file: ${e.message}`
					);
				}
			...
			if (
				...
				return reject(
					new Error(
						"PDF file was not created or is empty after LibreOffice conversion."
					)
				);
			}
			...
			try {
				if (
					mongoose.Types.ObjectId.isValid(
						correspondingAuthorId || req.user._id
					)
				) {
...
const createMergedPDFWithTable = async (
	...
	try {
		...
		if (formData.authors) {
			try {
				const authorIds = Array.isArray(formData.authors)
					? formData.authors
					: [formData.authors];

...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    const allowed =
      ...
      effectiveOrigins.some(
        (p) => p.startsWith("*.") && origin.endsWith(p.slice(1)),
      );
...
app.get("/test-libreoffice", async (req, res) => {
  ...
  try {
    ...
    // Try which
    try {
      results.path = execSync(
        'which libreoffice || which soffice || echo "not found"',
        {
          timeout: 5000,
          encoding: "utf8",
        },
...
app.get("/health", async (req, res) => {
  ...
  try {
    ...
    const { stdout: fontCount } = await execPromise("fc-list | wc -l", {
      timeout: 5000,
    });
...
const server = app.listen(PORT, async () => {
  ...
  cron.schedule("0 * * * *", async () => {
    console.log(
      `[PRODUCTION] Cron jobs running - ${new Date().toLocaleString()}`,
    );
